...
class LocalDiscovery extends Discovery {
  ...
  discover() {
    ...
    Factory.on('connected', this.stop.bind(this));
    Factory.on('disconnected', this._start.bind(this));
    Factory.on('nodeAdded', node => {
      this._hosts.push(node.host);
      this._hosts = _.uniq(this._hosts);
    });
    Factory.on('nodeRemoved', node => {
   ...
   */
  stop() {
    fs.unwatchFile(Factory.options.discovery.file);
  ...
  _watcher(curr, prev) {
    if (curr.mtime === 0) {
      return Factory.emit(new Error('Local discovery can\'t find file to watch for:', Factory.options.discovery.file));
    }
    if (curr.mtime != prev.mtime) {
      fs.readFile(Factory.options.discovery.file, 'utf8', (err, data) => {
        if (err) {
          return Factory.emit('error');
        }
        var nodes = data.split(/\s/);
        nodes = _.filter(_.difference(nodes, this._hosts), entry => entry.trim().length);
        if (nodes.length) {
          this._hosts.concat(nodes);
          Factory.emit('discovered', nodes);
        }
      });
    }
  }
